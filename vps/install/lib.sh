...
# apparmor_local_rules maintains a marked block of rules in the profile's
# local/ include, which Ubuntu profiles read for exactly this kind of site
# addition and which survives package upgrades. Rules are read from stdin. The
# block is replaced wholesale when it changes, so a later addition here reaches
# hosts that were installed earlier. A no-op where the profile is not shipped.
#
#   apparmor_local_rules usr.sbin.haproxy <<'EOF'
#   /etc/haproxy/conf.d/** r,
#   EOF
apparmor_local_rules() {
  local profile="$1" rules
  rules="$(cat)"
  local profile_file="/etc/apparmor.d/$profile" local_file="/etc/apparmor.d/local/$profile"
  [[ -f "$profile_file" ]] && command -v apparmor_parser >/dev/null || return 0

  local block new
  block="$(printf '# notification-hub begin — regenerated by the installer, edit outside this block\n%s\n# notification-hub end' "$rules")"
  ensure_dir "$(dirname "$local_file")" 0755 root:root
  touch "$local_file"
  new="$(python3 - "$local_file" "$block" <<'PYAA'
import re, sys
path, block = sys.argv[1], sys.argv[2]
text = open(path).read()
pat = re.compile(r'# notification-hub begin.*?# notification-hub end\n?', re.S)
if pat.search(text):
    text = pat.sub(lambda _: block + "\n", text)
else:
    text = text.rstrip("\n") + ("\n\n" if text.strip() else "") + block + "\n"
sys.stdout.write(text)
PYAA
)"
  if [[ "$new" != "$(cat "$local_file")" ]]; then
    log "updating AppArmor local rules for $profile"
    printf '%s\n' "$new" > "$local_file"
    apparmor_parser -r "$profile_file" || warn "could not reload the $profile AppArmor profile"
  fi
}
