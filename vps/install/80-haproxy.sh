...
# Ubuntu confines haproxy with an AppArmor profile that only permits reading
# /etc/haproxy/* — flat files — so the conf.d directory, the certificate
# directory and the chrooted log socket are all denied, even to root.
apparmor_local_rules usr.sbin.haproxy <<'EOF'
/etc/haproxy/conf.d/ r,
/etc/haproxy/conf.d/** r,
/etc/certs/proxy/combined/ r,
/etc/certs/proxy/combined/** r,
