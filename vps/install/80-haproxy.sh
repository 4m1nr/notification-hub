...
# directory and the chrooted log socket are all denied, even to root. The
# profile includes local/usr.sbin.haproxy for exactly this kind of site
# addition, and that file survives package upgrades. The block is replaced
# wholesale when it changes, so a later rule addition here reaches the host.
...
if [[ -f "$AA_PROFILE" ]] && command -v apparmor_parser >/dev/null; then
  AA_BLOCK="$(cat <<'EOF'
# notification-hub begin — regenerated by 80-haproxy.sh, edit outside this block
/etc/haproxy/conf.d/ r,
/etc/haproxy/conf.d/** r,
/etc/certs/proxy/combined/ r,
/etc/certs/proxy/combined/** r,
/var/lib/haproxy/dev/log w,
# notification-hub end
EOF
)"
  ensure_dir "$(dirname "$AA_LOCAL")" 0755 root:root
  touch "$AA_LOCAL"
  AA_NEW="$(python3 - "$AA_LOCAL" "$AA_BLOCK" <<'PYAA'
import re, sys
...
PYAA
  ...
  if [[ "$AA_NEW" != "$(cat "$AA_LOCAL")" ]]; then
    log "updating haproxy AppArmor local rules in $AA_LOCAL"
    printf '%s\n' "$AA_NEW" > "$AA_LOCAL"
